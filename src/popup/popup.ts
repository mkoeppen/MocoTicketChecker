...
class PopupHandler {
  ...
  constructor() {
    const ticketPrefixTable = document.querySelector<HTMLTableSectionElement>(
      ".mtc-options__prefix-table tbody",
    );
  ...
  save() {
    ...
    this.ticketPrefixTable.querySelectorAll("tr").forEach((row) => {
      ...
      const allowedTicketPrefixes = row
        .querySelector<HTMLInputElement>('[name="allowedTicketPrefixes"]')
        ?.value?.trim();
    ...
    document.body.addEventListener("click", (e: MouseEvent) => {
      if (
        (e.target as HTMLElement)
          .closest("button")
          ?.classList?.contains("mtc-remove-button")
      ) {
